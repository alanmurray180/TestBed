import { getMockStockData, searchMockTickers } from "./mock-data.js";
import { fetchAlphaVantageData, searchAlphaVantage } from "./alphavantage.js";
...
async function yahooFetch(url) {
  ...
  // Rate limited — do NOT clear session or retry (that makes it worse)
  if (res.status === 429) {
...
async function fetchStockData(ticker, provider = "yahoo") {
  try {
    if (provider === "alphavantage" && process.env.ALPHAVANTAGE_API_KEY) {
      return await fetchAlphaVantageData(ticker);
    }
    return await fetchYahooData(ticker);
  } catch (primaryErr) {
    // Primary provider failed — try the other as fallback
    try {
      if (provider === "alphavantage") {
        console.warn(`AlphaVantage failed (${primaryErr.message}). Trying Yahoo for ${ticker}...`);
        return await fetchYahooData(ticker);
      } else if (process.env.ALPHAVANTAGE_API_KEY) {
        console.warn(`Yahoo Finance failed (${primaryErr.message}). Trying AlphaVantage for ${ticker}...`);
        return await fetchAlphaVantageData(ticker);
      }
    } catch (fallbackErr) {
      console.warn(`Fallback also failed (${fallbackErr.message}). Falling back to demo data.`);
    }

    // Last resort — fall back to mock data
    const mock = getMockStockData(ticker);
    if (mock) {
      console.warn(`Using demo data for ${ticker}.`);
      return { _demo: true, ...mock };
    }
    throw new Error(
      `Could not fetch data for "${ticker}" and no demo data is available. (${primaryErr.message})`
    );
  }
}
...
async function analyzeStock(ticker, provider = "yahoo") {
  const data = await fetchStockData(ticker, provider);
...
async function searchTickers(query, provider = "yahoo") {
  ...
  let results;
  if (provider === "alphavantage") {
    // AlphaVantage selected — use AV search, fall back to mock
    if (process.env.ALPHAVANTAGE_API_KEY) {
      try {
        results = await searchAlphaVantage(query);
      } catch {
        results = searchMockTickers(query);
      }
    } else {
      results = searchMockTickers(query);
    }
  } else {
    // Yahoo selected — use Yahoo search, fall back to mock
    try {
