...
async function fetchStockData(ticker) {
  // Try live Yahoo Finance first, fall back to mock data if network is unavailable
  try {
    const modules = [
      "price",
      "summaryDetail",
    ...
    // Yahoo unavailable — try AlphaVantage as second source
    if (process.env.ALPHAVANTAGE_API_KEY) {
      try {
        console.warn(`Yahoo Finance unavailable (${yahooErr.message}). Trying AlphaVantage for ${ticker}...`);
        return await fetchAlphaVantageData(ticker);
      } catch (avErr) {
        console.warn(`AlphaVantage also failed (${avErr.message}). Falling back to demo data.`);
      }
    }
    ...
    throw new Error(
      `Could not fetch data for "${ticker}" and no demo data is available. (${yahooErr.message})`
    );
  }
}
...
async function searchTickers(query) {
  ...
  try {
    ...
    // Yahoo unavailable — try AlphaVantage search, then mock list
    if (process.env.ALPHAVANTAGE_API_KEY) {
      try {
