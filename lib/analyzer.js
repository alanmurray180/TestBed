...
async function fetchStockData(ticker) {
  ...
  try {
    ...
    if (mock) {
      console.warn(`Yahoo Finance unavailable (${liveErr.message}). Using demo data for ${ticker}.`);
      return { _demo: true, ...mock };
    }
    throw new Error(
      `Could not fetch data for "${ticker}" and no demo data is available. (${liveErr.message})`
    );
  }
}
...
async function searchTickers(query) {
  ...
  try {
    ...
    // Network unavailable — fall back to built-in ticker list
