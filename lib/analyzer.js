...
async function yahooFetch(url) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Yahoo Finance request failed (${res.status}): ${text.slice(0, 200)}`);
  }
  return res.json();
}
...
function computeTargetPrice(data) {
  ...
  if (targetMean) {
    models.push({
      ...
      target: targetMean,
      weight: 3,
      description: `Wall Street analyst consensus target is $${targetMean.toFixed(2)} (range: $${(targetLow || 0).toFixed(2)} – $${(targetHigh || 0).toFixed(2)}).`,
    });
  }
  ...
  if (trailingPE && forwardPE && trailingEps) {
    ...
    if (peTarget > 0) {
      models.push({
        ...
        target: peTarget,
        weight: 2,
        description: `Forward P/E of ${forwardPE.toFixed(1)}x applied to trailing EPS of $${trailingEps.toFixed(2)} yields a value of $${peTarget.toFixed(2)}.`,
      });
    }
  }
  ...
  if (pegRatio && trailingPE && trailingEps && pegRatio > 0) {
    ...
    if (pegTarget > 0) {
      models.push({
        ...
        target: pegTarget,
        weight: 1.5,
        description: `PEG ratio of ${pegRatio.toFixed(2)} suggests the stock is ${pegRatio < 1 ? "undervalued relative to growth" : "fully valued relative to growth"}, implying a fair value near $${pegTarget.toFixed(2)}.`,
      });
    }
  }
  ...
  if (revGrowth !== null) {
    ...
    models.push({
      ...
      target: growthTarget,
      weight: 1,
      description: `Revenue growth of ${(revGrowth * 100).toFixed(1)}% suggests a growth-adjusted value of $${growthTarget.toFixed(2)}.`,
    });
  }
  ...
  if (bookValue && currentPrice) {
    ...
    models.push({
      ...
      target: bookTarget,
      weight: 0.5,
      description: `Book value of $${bookValue.toFixed(2)} with a P/B ratio of ${priceToBook.toFixed(1)}x provides a floor valuation reference.`,
    });
  }
...
function buildRationale(ticker, data, pricing) {
  ...
  r += `## Summary\n\n`;
  r += `| Metric | Value |\n|---|---|\n`;
  r += `| **Current Price** | $${currentPrice.toFixed(2)} |\n`;
  r += `| **12-Month Target Price** | $${targetPrice.toFixed(2)} |\n`;
  r += `| **Implied Upside/Downside** | ${upside >= 0 ? "+" : ""}${upside.toFixed(1)}% |\n`;
  r += `| **Recommendation** | **${directionLabel}** |\n`;
  r += `| **Market Cap** | ${formatMarketCap(raw(price.marketCap))} |\n`;
  ...
  if (low52 && high52) {
    r += `| **52-Week Range** | $${low52.toFixed(2)} – $${high52.toFixed(2)} |\n`;
  }

  r += `\n## Valuation Models\n\n`;
  r += `The target price of **$${targetPrice.toFixed(2)}** is derived from a weighted blend of ${models.length} valuation approaches:\n\n`;

  for (const model of models) {
    r += `### ${model.name} (Weight: ${model.weight}, Target: $${model.target.toFixed(2)})\n\n`;
    r += `${model.description}\n\n`;
  }
...
function buildConclusion(ticker, pricing) {
  const { currentPrice, targetPrice, upside } = pricing;

  if (upside > 15) {
    return `Based on our multi-model analysis, **${ticker}** appears to be meaningfully undervalued at its current price of $${currentPrice.toFixed(2)}. Our blended target of **$${targetPrice.toFixed(2)}** represents **${upside.toFixed(1)}% upside potential**. The stock's fundamentals, combined with analyst expectations and growth trajectory, support a constructive outlook. Investors with appropriate risk tolerance may find this an attractive entry point.`;
  } else if (upside > 5) {
    return `Our analysis suggests **${ticker}** has moderate upside potential from its current price of $${currentPrice.toFixed(2)} to a target of **$${targetPrice.toFixed(2)}** (**+${upside.toFixed(1)}%**). While not deeply undervalued, the risk/reward profile is favorable for investors who believe in the company's continued execution.`;
  } else if (upside > -5) {
    return `At $${currentPrice.toFixed(2)}, **${ticker}** appears fairly valued relative to our blended target of **$${targetPrice.toFixed(2)}** (**${upside >= 0 ? "+" : ""}${upside.toFixed(1)}%**). We recommend a hold position, monitoring for catalysts that could shift the valuation materially in either direction.`;
  } else {
    return `Our analysis indicates **${ticker}** may be overvalued at its current price of $${currentPrice.toFixed(2)} relative to our target of **$${targetPrice.toFixed(2)}** (**${upside.toFixed(1)}%**). Investors may want to consider reducing exposure or waiting for a more attractive entry point.`;
  }
}
